...
(function($){
	...
	var defaults = {
		...
		mincontentheight: 30,    
		menuwidth: 2 // items before/after current
	}
		...
		return this.each(function(){
			...
			function spyonmenuitems($menu){
				...
				$menuitems.each(function(i){
					...
					targets.push( {$menuitem: $item, $des: $target, offsettop: targetoffset, height: targetheight, idx: targets.length} )
				})
			}
			...
			function highlightitem(){
				...
				if (shortlist.length > 0){
					currentIndex = shortlist[0].idx; // get the index of the new current target
					...
					// hide all other items than the 3 around the currrent
					for (i=0; i<targets.length;i++) {
						if ((i<currentIndex-o.menuwidth) || (i>currentIndex+o.menuwidth)) {
							targets[i].$menuitem.hide();
						}
						else {
							targets[i].$menuitem.show();
						}
					}
				}
			}
