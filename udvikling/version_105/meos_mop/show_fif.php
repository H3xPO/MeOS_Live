...
<META HTTP-EQUIV="refresh" content="60;?<?php echo $_SERVER['QUERY_STRING']; ?>"/>
...
<?php
  ...
  if ($r = mysql_fetch_array($res)) {
    ...
    print "<h1>$r[name] &ndash; ".date_format(date_create($r[date]),'d.m.Y')."</h1><p><i>Siden er sidst indlæst ".date('d.m.Y H:i:s', time())."</i></p>\n";
...
*/
  ...
  while ($r = mysql_fetch_array($res)) {
    ...
    print '<a href="'."$PHP_SELF?cmp=$cmpId&cls=$r[id]".'">'.$r['name']."</a><br/>\n";      

  }

  

/* kommende kode for klubber
  ...
  $sql = "SELECT DISTINCT org.id, org.name ".
         ...
         "WHERE cmp.cid = '$cmpId' ORDER BY org.name";
  ...
  while ($r = mysql_fetch_array($res)) {
    ...
    print '<a href="'."$PHP_SELF?org=$r[id]".'">'.$r['name']."</a><br/>\n";      
      ...
      if ($radio!='') {
        ...
        if ($radio == 'finish') {
          ...
          $sql = "SELECT team.id AS id, cmp.name AS name, team.name AS team, radio.rt AS time, 1 AS status, ".
                   ...
                   "AND team.cid = '$cmpId' AND m.cid = '$cmpId' AND cmp.cid = '$cmpId' AND radio.cid = '$cmp' ".
       ...
       if ($radio!='') {
         ...
         if ($radio == 'finish') {
           ...
           $sql = "SELECT team.id AS id, cmp.name AS name, team.name AS team, radio.rt AS time, 1 AS status ".
                   ...
                   "AND radio.cid = '$cmpId' AND m.cid = '$cmpId' AND team.cid = '$cmpId' AND cmp.cid = '$cmpId' AND radio.cid = '$cmp' ".
